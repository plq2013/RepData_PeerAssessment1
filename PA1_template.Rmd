...
1. Make a histogram of the total number of steps taken each day
    ...
    ```{r, echo=TRUE}
        ...
        hist(stepsPerDay$steps, xlab="total number steps taken per day",
...
1. Time series plot of the 5-minute interval and the average number of steps taken, averaged across all days
    ...
    ```{r, echo=TRUE}
        ...
        stepsPerInterval <- data.table(data.raw)[, lapply(.SD, mean, na.rm=TRUE), by = interval]
        xyplot(steps~interval, data= stepsPerInterval, type="l", xlab="Identified intervals", ylab="Averaged number steps across all days")
...
2. Make a panel plot containing a time series plot (i.e. type = "l") of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all weekday days or weekend days (y-axis). The plot should look something like the following, which was creating using simulated data:
    ```{r, echo=TRUE}
    stepsPerInterval2=data.table(data.filled)[, lapply(.SD, mean), by = list(interval ,day)]
